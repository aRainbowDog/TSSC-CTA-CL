...
import torch.nn.functional as F  # 确保导入了 functional
...
class SDPA_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 使用 PyTorch 原生高效注意力计算
        # 它会自动处理 scale，并根据显存情况选择 FlashAttention 或 Memory Efficient 模式
        x = F.scaled_dot_product_attention(
            q, k, v, 
            dropout_p=self.attn_drop.p if self.training else 0.0,
            is_causal=False # 对于 DiT 这种双向注意力，设为 False
        )
...
class MVIFBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, mode='video', num_frames=16, **block_kwargs):
        ...
        self.attn = SDPA_Attention(hidden_size, num_heads=num_heads, qkv_bias=True, **block_kwargs)
        ...
        if self.mode == 'video':
            ...
            self.temporal_attn = SDPA_Attention(hidden_size, num_heads=num_heads, qkv_bias=True)
